...
//
//        val channel = NotificationChannel(Notifications.Channel.DEFAULT_ID, getString(R.string.channel_name), NotificationManager.IMPORTANCE_HIGH)
...
//                            val data = Lms(className, sbn.postTime, Lms.TYPE_HOMEWORK, timeFormat.parse(startTime)?.time ?: 0L, timeFormat.parse(endTime)?.time ?: 0L, isRenewAllowed = true,isFinished = false, -1, -1,  homework_name)
//
...
//                                    val notiIntent = Intent(applicationContext, TimeReceiver::class.java).apply { putExtra(Intents.Key.ITEM_ID, id) }
//
...
//                                        val pendingIntent = PendingIntent.getBroadcast(applicationContext, PendingReq.LAUNCH_NOTI + id * 100 + index + 1, notiIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
...
//                                        val clickIntent = Intent(applicationContext, MainActivity::class.java).apply { putExtra(Intents.Key.ITEM_ID, id) }
//                                        val clickPendingIntent = PendingIntent.getActivity(applicationContext, PendingReq.LAUNCH_NOTI + id, clickIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
//                                        val deleteIntent = Intent(applicationContext, DeleteMissReceiver::class.java).apply { putExtra(Intents.Key.ITEM_ID, id) }
//                                        val deletePendingIntent = PendingIntent.getBroadcast(applicationContext, PendingReq.DELETE + id, deleteIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
//
//                                        NotificationCompat.Builder(applicationContext, Notifications.Channel.DEFAULT_ID).apply {
...
//                                                nm.notify(Notifications.Id.REGISTER + id, build())
//                                            }
//                                        }
//                                    }
//                                }
...
//                                    notiIntent.putExtra(Intents.Key.TRIGGER, triggerTime)
//                                    notiIntent.putExtra(Intents.Key.TIME, i)
//                                    val pendingIntent = PendingIntent.getBroadcast(applicationContext, PendingReq.LAUNCH_NOTI + id.toInt() * 100 + index + 1, notiIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
//                                    if (triggerTime > System.currentTimeMillis()) am.setExact(AlarmManager.RTC_WAKEUP, triggerTime, pendingIntent)
//                                }
//
//                                if (sharedPreferences.getBoolean(SharedKey.SET_REGISTER_ALERT, true)) {
//                                    val clickIntent = Intent(applicationContext, MainActivity::class.java).apply { putExtra(Intents.Key.ITEM_ID, id) }
//                                    val clickPendingIntent = PendingIntent.getActivity(applicationContext, PendingReq.LAUNCH_NOTI + id.toInt(), clickIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
//                                    val deleteIntent = Intent(applicationContext, DeleteMissReceiver::class.java).apply { putExtra(Intents.Key.ITEM_ID, id) }
//                                    val deletePendingIntent = PendingIntent.getBroadcast(applicationContext, PendingReq.DELETE + id.toInt(), deleteIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
//
//                                    NotificationCompat.Builder(applicationContext, Notifications.Channel.DEFAULT_ID).apply {
//                                        setContentTitle(className)
...
//
//                                        if (!sharedPreferences.getBoolean(SharedKey.DND_CHECK, false) || (!Utils.isDNDTime(dndStartData, dndEndData, nowInt))) {
//                                            nm.notify(Notifications.Id.REGISTER + id.toInt(), build())
//                                        }
//                                    }
//                                }
//                            }
...
//                            val data = Lms(className, sbn.postTime, Lms.TYPE_LESSON, -1, timeFormat.parse(endTime)?.time ?: 0L, isRenewAllowed = true, isFinished = false, week.toInt(), lesson.toInt(), "#NONE")
//
//                            GlobalScope.launch(Dispatchers.IO) {
//                                if (lmsDatabase?.getDao()?.checkIsAlreadyInDB(data.className, data.week, data.lesson, data.homework_name) != true) {
//                                    val id = lmsDatabase?.getDao()?.add(data)?.toInt() ?: -1
...
//                        if (matchResult != null) {
//                            val (week, lesson, endTime) = matchResult.destructured
//                            val data = Lms(className, sbn.postTime, Lms.TYPE_SUP_LESSON, -1, timeFormat.parse(endTime)?.time ?: 0L, isRenewAllowed = false, isFinished = false, week.toInt(), lesson.toInt(), "#NONE")
//
//                            GlobalScope.launch(Dispatchers.IO) {
//                                if (lmsDatabase?.getDao()?.checkIsAlreadyInDB(data.className, data.week, data.lesson, data.homework_name) != true) {
//                                    val id = lmsDatabase?.getDao()?.add(data)?.toInt() ?: -1
...
//                        if (matchResult != null) {
//                            val (week, lesson, endTime) = matchResult.destructured
//                            val data = Lms(className, sbn.postTime, Lms.TYPE_SUP_LESSON, -1, timeFormat.parse(endTime)?.time ?: 0L, isRenewAllowed = true, isFinished = false, week.toInt(), lesson.toInt(), "#NONE")
//
//                            var id: Long
//
//                            GlobalScope.launch(Dispatchers.IO) {
...
//                            val data = Lms(className, sbn.postTime, Lms.TYPE_ZOOM, 0L, timeFormat.parse(endTime)?.time ?: 0L, isRenewAllowed = true, isFinished = false, -1, -1, homework_name)
//
//                            GlobalScope.launch(Dispatchers.IO) {
//                                if (lmsDatabase?.getDao()?.checkIsAlreadyInDB(data.className, data.week, data.lesson, data.homework_name) != true) {
//                                    val id = lmsDatabase?.getDao()?.add(data)?.toInt() ?: -1
...
//                        if (matchResult != null) {
//                            val (homework_name, startTime, endTime) = matchResult.destructured
//                            val data = Lms(className, sbn.postTime, Lms.TYPE_TEAMWORK, timeFormat.parse(startTime)?.time ?: 0L, timeFormat.parse(endTime)?.time ?: 0L, isRenewAllowed = true,isFinished = false, -1, -1,  homework_name)
//
//                            GlobalScope.launch(Dispatchers.IO) {
//                                if (lmsDatabase?.getDao()?.checkIsAlreadyInDB(data.className, data.week, data.lesson, data.homework_name) != true) {
//                                    val id = lmsDatabase?.getDao()?.add(data)?.toInt() ?: -1
...
//                        if (matchResult != null) {
//                            val (homework_name, startTime, endTime) = matchResult.destructured
//                            val data = Lms(className, sbn.postTime, Lms.TYPE_EXAM, -1, timeFormat.parse(startTime)?.time ?: 0L, isRenewAllowed = true,isFinished = false, -1, -1,  homework_name)
//
//                            GlobalScope.launch(Dispatchers.IO) {
//                                if (lmsDatabase?.getDao()?.checkIsAlreadyInDB(data.className, data.week, data.lesson, data.homework_name) != true) {
//                                    val id = lmsDatabase?.getDao()?.add(data)?.toInt() ?: -1
