...
class TimeReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        nm.createNotificationChannel(NotificationChannel(ChannelId.DEFAULT_ID, context.getString(R.string.channel_name), NotificationManager.IMPORTANCE_HIGH))
        nm.createNotificationChannel(NotificationChannel(ChannelId.TIME_REMINDER_ID, context.getString(R.string.channel_time_reminder), NotificationManager.IMPORTANCE_HIGH))
        ...
        GlobalScope.launch(Dispatchers.IO) {
            val item = lmsDatabase?.getDao()?.get(id) ?: LMSEntity("", 0L, 0, 0L, 0L, false, false, -1, -1, "")
            Log.d(TAG, "${item}")
            when (item.type) {
                LMSEntity.TYPE_LESSON -> {
                    if (abs(System.currentTimeMillis() - triggerTime) <= 3000 && !item.isFinished && (time in lecHoursOn)) {
                        if (item.className != "") {
                            NotificationCompat.Builder(context, ChannelId.TIME_REMINDER_ID).apply {
                                val clickIntent = Intent(context, MainActivity::class.java).apply { putExtra(IntentKey.ITEM_ID, id) }
                                val clickPendingIntent = PendingIntent.getActivity(context, PendingIntentReqCode.LAUNCH_NOTI + id.toInt(), clickIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
                                val markIntent = Intent(context, MarkFinishReceiver::class.java).apply { putExtra(IntentKey.ITEM_ID, id) }
                                val pendingIntent = PendingIntent.getBroadcast(context, PendingIntentReqCode.MARKING + id.toInt(), markIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
                                ...
                                if (!sharedPreferences.getBoolean(SharedKey.DND_CHECK, false) || (!MyUtils.isDNDTime(dndStartData, dndEndData, nowInt))) {
                                    nm.notify(NotificationId.TIME_REMINDER + id.toInt(), build())
                                }
                            }
                        }
                    }
